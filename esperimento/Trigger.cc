...
Trigger::singletonMap_t Trigger::singleton;
...
void Trigger::AddTrack( const G4Track * aTrack )
{
	...
	// if (thisEventTotEM[volCopyNum-1]>3*MeV){
		...
		if (start!=-1 && (aTrack->GetGlobalTime()-start)>100*ns){
		    ...
		    HistoManager::GetInstance()->HistoFill((time-start)/microsecond, fDecayTime);
		    if (mom.y()>0)  HistoManager::GetInstance()->HistoFill(time/microsecond, fDecayTimeForward);
		    else HistoManager::GetInstance()->HistoFill(time/microsecond, fDecayTimeBackward);
		}
	// }
...
void Trigger::AddEDepEM( G4double edep, G4int volCopyNum, G4double time)
{
...
void Trigger::PrepareNewEvent(const G4Event* )
{
	// histoManager = histo;
...
void Trigger::EndOfEvent(const G4Event* anEvent){
	Trigger::singleton.erase(anEvent->GetEventID());
}
