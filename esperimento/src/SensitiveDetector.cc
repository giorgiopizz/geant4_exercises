...

{
  ...
  G4String myCollectionName = "ScintHitCollection";
...
G4bool SensitiveDetector::ProcessHits(G4Step *step, G4TouchableHistory *)
{
  ...
  if (thistrack){
      if (thistrack->GetCreatorProcess()){
          // if ((thistrack->GetCreatorProcess()->GetProcessType()==fDecay || thistrack->GetCreatorProcess()->GetProcessType()==fHadronic) && (copyNo>=1 && copyNo<=3)) {
          if (copyNo>=1 && copyNo<=3) {
              MyHit * hit = new MyHit();
              G4int pt = -1;
  ...
  //
  //
  // // hitMap.insert( std::make_pair(HDGeindex,bla) );
...
//
//
// // così c'è una sola hit per copynumber e quindi per sensitie detector
// // questa, alla fine conterrà tutta l'energia depositata dentro al rivelatore
//
