...
{
	for (int i;i<3;i++) activation_times[i] = -1;
}
...
void Analysis::AddEDepEM( G4double edep, G4int volCopyNum, G4double time)
{
	...
	if (time>=0*ns){
		...
		if (thisEventTotEM[volCopyNum-1]>0.05*MeV && activation_times[volCopyNum-1]==-1){

			// salvo il tempo di attivazione
			activation_times[volCopyNum-1] = time;

			if (volCopyNum==3 && activation_times[0]!=-1){
				// G4cout << "doppia" << G4endl;
				// mi ha attivato il terzo scintillatore ed era già attivo il primo => doppia
				doppie+=1.;
			}

			if (volCopyNum==3 && activation_times[0]!=-1 && activation_times[1]!=-1 ){
				// G4cout << "tripla" << G4endl;
				// mi ha attivato il terzo e il primo ed il secondo erano già attivi => tripla
				triple +=1.;
			}

		//
		// if (volCopyNum==1 && activation_times[0]==-1){
...
void Analysis::EndOfRun(const G4Run* aRun)
{
	...
	//   delete histos[i];
	// histos.clear();
	G4cout << "triple: " << triple << "\t" << doppie << G4endl;
	G4cout<< "fine run, geometrico ottenuto: " << triple / doppie << G4endl;
}
